<template>
  <main class="container">
    <div class="button" @click="toggleModal">Add City</div>
    <BaseModal :modalActive="modalActive" @close-modal="toggleModal">
      <div class="search__block">
        <input type="text" v-model="searchQuery" @input="getSearchResults" placeholder="Search city"
          class="search__input" />
        <div class="button" @click="addCity">Add City</div>
        <ul class="search__list" v-if="mapboxSearchResults">
          <p class="search__text" v-if="searchError">
            Sorry, something went wrong, please try again.
          </p>
          <p class="search__text" v-if="!searchError && mapboxSearchResults.length === 0">
